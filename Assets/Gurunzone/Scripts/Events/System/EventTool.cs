    ...
    public class EventTool 
    {
        ...
        public static string GetCodeValue(string code)
        {
            ...
            if (code.Length >= 3 && code.Contains(":"))
            {
                ...
                if (type.ToLower() == "i")
                    output = SaveEventData.Get().GetCustomInt(id).ToString();
                if (type.ToLower() == "f")
                    output = SaveEventData.Get().GetCustomFloat(id).ToString();
                if (type.ToLower() == "s")
                    output = SaveEventData.Get().GetCustomString(id);
                if (type.ToLower() == "t")
                    output = EventManager.Get().GetTargetTitle(id);
            }
